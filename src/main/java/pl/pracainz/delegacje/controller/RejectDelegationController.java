...
@Controller
public class RejectDelegationController {
    @Autowired
    ...
    @RequestMapping(value = "/owner/reject-delegation/{id}", method = RequestMethod.GET)
    public ModelAndView approveDelegation(@PathVariable int id, RedirectAttributes redirectAttributes) {
        ...
        delegation.setDelegationStatus(delegationStatusService.getByStatus("REJECTED"));
        redirectAttributes.addFlashAttribute("successMessage", "Odrzucono delegację " + "\"" + delegation.getTitle() + "\"");
